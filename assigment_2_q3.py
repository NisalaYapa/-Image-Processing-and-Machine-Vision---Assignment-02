...
def select_points(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and len(architectural_points) < 4:
        ...
        cv2.circle(architectural_image, (x, y), 5, (0, 255, 0), -1)
        cv2.imshow("Architectural Image", architectural_image)
    ...
    elif event == cv2.EVENT_LBUTTONDOWN and len(flag_points) < 4:
        flag_points.append((x, y))
        cv2.circle(flag_image, (x, y), 5, (0, 255, 0), -1)
        cv2.imshow("Flag Image", flag_image)
...
architectural_image = cv2.imread("notredam.jpeg")
